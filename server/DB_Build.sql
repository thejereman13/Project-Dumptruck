...
DELIMITER $$
CREATE FUNCTION FindGIDUser (guid TEXT) RETURNS VARCHAR(36)
BEGIN
    DECLARE uid VARCHAR(36);
    IF EXISTS (SELECT 1 FROM Users WHERE GID = guid) THEN
        SELECT UserID INTO uid FROM Users WHERE GID = guid;
    ELSE
        SET uid = "";
    END IF;
    RETURN uid;
